...
input_rho_A <- 0
input_mu_B <- c(1, 2, 5)
input_sigma_B <- seq(0.001, 3.5, length.out = 10)
input_rho_B <- 0
input_h <- 1
...
if(parallel) {
  ...
  print(system.time(out_abds  <- IterateOverSatCorrParams(iterated_params = cur_params, settings = settings)))
  ...
  # running the simulations
  print(system.time(out_abds  <- IterateOverSatCorrParams(iterated_params = iterated_params, settings = settings)))
