...
input_rho_A <- 0
input_mu_B <- c(1, -1)
input_sigma_B <- seq(0.01, 0.8, length.out = 10)
input_rho_B <- 0
input_h <- 0
...
if(parallel) {
  ...
  print(system.time(out_abds  <- IterateOverConstrainedParams(iterated_params = cur_params, settings = settings)))
  ...
  # running the simulations
  print(system.time(out_abds  <- IterateOverConstrainedParams(iterated_params = iterated_params, settings = settings)))
